		...
		try {
			if (mapData == null || mapData.equals("")) {
				...
				if (flag == 1) {
					...
					Route newRoute = new Route();// routeMap.get(routeName);//新路线
					...
					newRoute.setCreateDate(timestamp);
					...
					if (existRoutes.size() > 0) {
						...
						if (routeMap.containsKey(routeName)) {
							//路线存在，进行更新
							int tag = routeServiceImpl.updateByRouteName(routeName,newRoute);
